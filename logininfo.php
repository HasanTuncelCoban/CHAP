...
<li><button style="background:transparent url(logo/adminpnlicon/adminmngr.png) no-repeat left center; width:220px; height:40px; " class="menubottomSTLY"onclick="location.href='adminmngr.php';" > ADMIN MANAGER</button></li>
    <li><button style="background: transparent url(logo/adminpnlicon/userico.png) no-repeat left center; width:220px; height:40px;" class="menubottomSTLY" onclick="location.href='usrinfo.php';"> &emsp;USER INFORMATION</button></li>
     <li><button style="background: transparent url(logo/adminpnlicon/login.png) no-repeat left center; width:220px; height:40px; color:#FFFFFF;
...
$kayit_sayisi=mysqli_fetch_array(mysqli_query($conn,"SELECT COUNT(*) FROM $log_in"));
...
$sorgu=mysqli_query($conn,"SELECT * FROM $log_in LIMIT $nerden, $nereye");
...
$Userid=$veri['usrid'];
$LOGDATETIME=$veri['log_time_date'];
$LOGINIP=$veri['ip'];
$TYPEOFBROWSER=$veri['type_of_browser'];
$TYPEOFOS=$veri['opsystem'];
echo "
<tr><td >$Userid</td> <td>$LOGDATETIME</td>  <td>$LOGINIP</td>   <td>$TYPEOFBROWSER </td> <td >$TYPEOFOS</td>
